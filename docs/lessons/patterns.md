...
## §병렬처리 (Parallel Tool Execution)

**목적**: 독립 작업을 순차 처리해 불필요하게 시간을 낭비하는 패턴 차단.

**핵심 원칙**: 서로 의존성 없는 tool 호출은 단일 메시지에 묶어 동시 실행.

**병렬 실행 가능 패턴** (단일 메시지에 multiple tool 호출):
- 서로 다른 파일 Read × N
- 서로 다른 파일 Edit × N (파일 간 의존성 없을 때)
- Grep 여러 파일 동시 검색
- SSH 호출 + 로컬 파일 Read 동시
- 여러 에이전트(Explore/Plan/Agent) 동시 spawn

**순차 실행 필요 패턴** (이전 결과가 다음 입력):
- Read → (읽은 내용 기반) Edit
- SSH 실행 → 결과 파싱 → 다음 SSH
- Plan 에이전트 결과 → 코드 작성

**자가 검증 체크**:
- [ ] 이 tool 호출들이 서로 결과를 공유하는가? (예: A 결과 → B 입력)
- [ ] 아니라면 → 단일 메시지 multiple tool 호출로 묶었는가
- [ ] 같은 파일 2번 Edit이면 첫 번째 완료 후 두 번째 실행 (파일 상태 의존)

**연관 실패**: failures.md ㊿(ops 알림 순차 Edit)

**hookify 보강**: 없음 (tool 호출 패턴은 Claude 행동 습관 — hook 트리거 어려움)

---

## §ops알림언어 (Ops Alert Language for Non-Technical User)

**목적**: 텔레그램·이메일 ops 알림을 기술 용어로 작성해 비전공자가 읽고 즉시 대응 불가한 패턴 차단.

**핵심 원칙**: 승현님은 코드·인프라 용어 없이도 즉시 판단하고 행동할 수 있어야 함.

**금지 용어** (비전공자 관점에서 의미 없음):
- Python 라이브러리: `gspread.exceptions.APIError`, `RuntimeError`, `AttributeError`
- 인프라 개념: `atomic swap`, `quota exceeded`, `cron job`, `API endpoint`, `OAuth`
- 코드 위치: `repurchase_report.py:1234`, `_find_tab_with_retry()`

**허용 구조** (3단계):
1. **무슨 일** — 사용자 관점에서 어떤 기능이 안 됐는지 (예: "오늘 자동 분석이 안 돌았어요")
2. **왜** — 기술 용어 없이 이유 (예: "구글 시트가 바쁜 시간에 요청이 너무 많았어요")
3. **행동 단계** — 실행 경로 아닌 메뉴/버튼 기준 (예: "Apps Script → ⏱ 시계 아이콘 → 트리거 확인")

**작성 예시**:
| ❌ 기술 용어 | ✅ 비전공자 언어 |
|---|---|
| `gspread APIError [429] Quota exceeded` | 구글 시트 접속 요청이 잠시 한도 초과 → 자동 재시도 중 |
| `atomic swap 실패 — worksheets() 캐시 미갱신` | 시트 구조 변경 직후 탭 인식 실패 → 재시도로 해소 |
| `repurchase_report.py RuntimeError` | 재구매 자동 분석 전체 멈춤 → Claude한테 점검 요청 |

**자가 검증 체크**:
- [ ] Python 라이브러리명이 메시지에 없는가
- [ ] "cron", "API", "atomic", "quota" 단어 없는가
- [ ] 행동 단계가 메뉴/버튼 기준인가 (코드 파일 경로 아님)
- [ ] 메시지만 보고도 다음 행동을 알 수 있는가
