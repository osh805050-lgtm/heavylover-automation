...
## 2. 원인 가설
- 변화의 원인을 가능한 한 다층적으로 (외부 요인·내부 운영·계절성)

## 3. 다음 주 우선순위 3개
- 각 항목: 액션 / 예상 효과 / 검증 방법 / 기간
- 구체 수치·기간 포함, 모호한 권고 금지

규칙: 입력 JSON 숫자만 사용. 800~1500자. 한국어. 불릿·표 적극."""

SYSTEM_SKEPTIC = """당신은 10년차 회의주의자 분석가다. 동료 전략가의 1주일 분석 초안을 비판적으로 검토한다.
...
SYSTEM_STRATEGIST_V2 = """당신은 10년차 D2C 전략가다. 자신의 1차 초안에 대한 회의주의자 비판을 받았다. 이를 반영해 최종 전략 리포트를 작성한다.

작성 형식:
## 1. 이번 주 핵심 변화 (3개) — 확신도 명시
- 각 변화에 (확신도: 높음/중간/낮음) 표시

## 2. 원인 가설 — 회의주의자 반박 반영
- 단정 가능한 것과 가설로 남기는 것 구분

## 3. 다음 주 우선순위 3개 — 검증 가능성 보강
- 각 권고: 액션 / 예상 효과 / 검증 방법 / 검증 기간 / 실패 시 대안
...
위 데이터로 1차 전략 리포트 초안 작성."""
    v1 = call_claude(SYSTEM_STRATEGIST_V1, user_v1)
    if not v1:
        return None
...
위 초안을 비판적으로 검토. 과대해석·누락·샘플 한계·약한 권고 지적."""
    critique = call_claude(SYSTEM_SKEPTIC, user_v2)
    if not critique:
        return v1  # 1차만 발송
    print(f"[2차] 완료 ({len(critique)}자)")

    # 3차: 전략가 최종본
    print("[3차] 전략가 최종본 작성 중...")
    user_v3 = f"""당신의 1차 초안:
---
{v1}
---
...
비판을 반영해 최종 리포트 작성. 확신도·검증 방법·데이터 한계 명시."""
    final = call_claude(SYSTEM_STRATEGIST_V2, user_v3)
    if not final:
        return v1  # 폴백
    print(f"[3차] 완료 ({len(final)}자)")
